...
class FlappyDragon extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("dragon", "img/dragao.png", {
      frameWidth: this.player.width,
      frameHeight: this.player.height,
    });
  ...
  create() {
    ...
    this.cols.col1_obj = this.add
      .image(this.cols.x, this.cols.y, "colUpper")
      .setOrigin(0, 0);
    this.cols.col2_obj = this.add
      .image(
        this.cols.x,
        this.cols.y + this.cols.height + this.cols.space,
        "colBottom"
      )
      .setOrigin(0, 0);
    ...
    this.player.obj = this.physics.add.sprite(170, 130, "dragon").setScale(0.8);
    ...
    this.highScoreText = this.add.text(
      ...
      { fontSize: "20px", fill: "#000", align: "right" }
    );
    this.highScoreText.x =
      this.game.config.width - this.highScoreText.width - 15;
  ...
  update() {
    ...
    if (this.cursors.left.isDown) this.player.obj.setX(this.player.obj.x - 5);
      ...
      this.player.obj.setY(
        this.player.obj.y - this.game.config.physics.arcade.gravity.y
      );
    ...
    if (!this.gameControls.current_col_scored) {
      if (
        this.player.obj.x - this.player.width / 2 >
        this.cols.x + this.cols.width
      ) {
